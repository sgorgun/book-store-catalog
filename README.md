...
One of the possible designs is shown on the diagram below.
...
* Property
    * The class must have the property `Code` to access the ISBN code. The property must have only the public [get accessor](https://docs.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/using-properties#the-get-accessor) that must return a value of the `code` field. The property type must be `string`.
* Constructor
    * The class must have a public [constructor](https://docs.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/constructors) with `isbnCode` parameter that validates the ISBN code and initializes the `code` field with the given argument.
...
A valid ISBN code must follow these rules:
* A code must have ten characters.
* Every character is either a digit or "X" character (represents 10).
...
* Properties
    * The class must have the public property `Amount` to access the `amount` field. The property must have both get and set accessors. The property type must be `decimal`.
    * The class must have the public property `Currency` to access the `currency` field. The property must have both get and set accessors. The property type must be `string`.
...
* Constructors
    ...
    * The class must have a constructor with two parameters - `amount` and `currency`. The constructor must validate the constructor arguments and initialize the `amount` and `currency` fields if both arguments are valid.
    * The constructor with parameters must use the `ThrowExceptionIfAmountIsNotValid` and `ThrowExceptionIfCurrencyIsNotValid` methods to validate the arguments.
...
The amount argument is valid, if the value is greater or equals zero.

The currency argument must follow these rules:
* A currency must have three characters.
...
A valid ISNI code must follow these rules:
* A code must have sixteen characters.
* Every character is either a digit or "X" character (represents 10).
...
* Properties
    ...
    * All properties must have a public get and a private set accessors. 
* Constructor
...
* Constructor
    ...
    * The class must have a constructor to initialize an object with the `publication`, `price` and `amount` parameters. The `publication` parameter must have the `BookPublication` type. The `price` parameter must have the `BookPrice` type.
    ...
    * To remove the code duplication, use the constructor chaining approach.
* Properties
    * The class must have the `Publication` property to get and set the `publication` field. The property type must be `BookPublication`.
    * The class must have the `Price` property to get and set the `price` field. The property type must be `BookPrice`.
    * The class must have the `Amount` property to get and set the `amount` field. The property type must be `int`.
    * All properties must have a public get and set accessors.
